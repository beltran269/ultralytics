...
| `match_thresh`      | `0.0-1.0` (IoU) or `>0` (L2)                  | Threshold for matching tracks. For IoU: cost threshold in `[0, 1]`. For L2: pixel distance threshold (e.g., `100`).                                    |
| `fuse_score`        | `True`, `False`                               | Determines whether to fuse confidence scores with IoU distances before matching. Only used with `iou` distance metric.                                 |
| `distance_metric`   | `iou`, `l2`                                   | Distance metric for track association. `iou` uses Intersection over Union cost, `l2` uses Euclidean distance between bbox centers in pixels.           |
